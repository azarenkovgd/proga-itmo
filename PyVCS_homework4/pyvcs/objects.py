...
def hash_object(data: bytes, fmt: str, write: bool = False) -> str:
    ...
    if write:
        git_dir = repo_find()
        obj_dir = pathlib.Path(str(git_dir) + "/objects/" + obj_hash[:2])
...
def resolve_object(obj_name: str, git_dir: pathlib.Path) -> tp.List[str]:
    if not 4 <= len(obj_name) <= 40:
        raise Exception(f"Not a valid object name {obj_name}")
    dir_name = obj_name[:2]
    obj_file = obj_name[2:]
    obj_dir = str(git_dir) + "/objects/" + dir_name
...
def find_object(obj_name: str, git_dir: pathlib.Path) -> str:
    pass


def read_object(sha: str, git_dir: pathlib.Path) -> tp.Tuple[str, bytes]:
    obj_name = resolve_object(sha, git_dir)[0]
    assert len(resolve_object(sha, git_dir)) == 1
    obj_dir = pathlib.Path(obj_name[:2])
    obj_file_name = pathlib.Path(obj_name[2:])
    path = git_dir / "objects" / obj_dir / obj_file_name
...
def cat_file(obj_name: str, pretty: bool = True) -> None:
    git_dir = repo_find()
    obj_type, content = read_object(obj_name, git_dir)
    ...
    if obj_type == "blob":
        ...
        for entry in tree_entries:
            ...
            if len(mode) != 6:
                mode = "0" + mode
            tree_pointer_type, _ = read_object(entry[1], git_dir)
